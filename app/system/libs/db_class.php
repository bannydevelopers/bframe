...
class db{
        ...
        if (self::$instance === null) {
            ...
            try{
                if($config['dbdriver'] == 'mysql'){
                    $connstr = $config['dbdriver'].':host='.$config['hostname'].';dbname='.$config['db_name'];
                    self::$conn = new PDO($connstr,  $config['username'], $config['password'], $options);
                }
            }
            catch(PDOException $e){
                self::$qry = self::$qry ? self::$qry : 'connection';
                self::$error[md5(self::$qry)]  = ['message'=>$e->getMessage(), 'query'=>self::$qry]; // Save error
            }
        }
        return self::$instance;
    }
    public function query($qry, $data = null){
        self::$qry = $qry;
    ...
    public function fetch(){
        $then = $this->bind_data();
        return $then ? $then->fetch(PDO::FETCH_ASSOC) : [];
    }
    ...
    public function fetchAll(){
        $then = $this->bind_data();
        return $then ? $then->fetchAll(PDO::FETCH_ASSOC) : [];
    }
